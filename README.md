...
This package also provides a standalone OpenAI-compatible API server (`server.py`), allowing you to use `gemini-webapi` as a drop-in replacement for OpenAI endpoints in other applications.

### Server Dependencies

To run the server, you need to install the optional `server` dependencies:

```sh
pip install "gemini_webapi[server]"
```

### Starting the Server

The server requires the same cookies JSON file as the CLI tool for authentication:

```sh
python server.py --cookies-json cookies.json --port 8000
```

Once running, you can connect to it using any standard OpenAI client (e.g., the official `openai` Python package or LangChain) by pointing the base URL to `http://127.0.0.1:8000/v1` and providing a dummy API key.

### Supported Endpoints

- **`GET /v1/models`**: Lists available Gemini models dynamically discovered from your account.
- **`POST /v1/chat/completions`**: Generates text responses, with full support for streaming (`stream: true`). Context handling is stateless, meaning the entire prompt history you send is combined and passed to Gemini on each request.
- **`POST /v1/images/generations`**: Generates images with Gemini. Requesting multiple images (`n > 1`) will return up to that amount of images if they are generated by the model.
